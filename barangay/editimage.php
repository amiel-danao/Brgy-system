...
if (isset($_GET['id']))
   {
        ...
        $MYsql = "SELECT `brgy_code` , `picture`, `post_validation` FROM `brgy_official_tbl` WHERE `brgy_id`=? ";
...
//   {
// 	    $id = filter_var($_GET['id'], FILTER_SANITIZE_NUMBER_INT);
// 	    $MYsql = "SELECT `brgy_code` , `picture` FROM `brgy_official_tbl` WHERE `brgy_id`=? ";
// 		$MYstmt = $db->prepare($MYsql);
// 		$MYstmt->bind_param('i',$id);
// 		$MYstmt->execute();
// 		$Result = $MYstmt->get_result();
...
?>
		<div class="text-center mb-3">
    		<img src="../admin/barangayimages/<?php echo $code ?>/<?php echo $picture ?>" class="w-25 img-fluid" style="height: 150px;"> 
    	</div>
		<form action="<?php echo htmlspecialchars("include/function/barangay_official_function.php"); ?>" method="post" enctype="multipart/form-data" onsubmit="return validateimage()" name="barangay_officials">
		    ...
		    <div class="row form-group">
	            ...
	            <div class="col-12 col-md-9">
	                <input type="file" id="new_official_image" name="new_official_image" class="form-control" accept="image/png, image/jpeg" required="required" value="<?php echo $picture ?>">
	            </div>
       		</div>
