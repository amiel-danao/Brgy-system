  <?php 
        ...
        if (isset($_GET['urecorda']))
            {
                $key = $_GET['urecorda'];
                $Mysql = "SELECT * FROM `bdrrmf_a_tbl` WHERE `id` = ? ";
                $Mystmt = $db->prepare($Mysql);
                $Mystmt->bind_param('i',$key);
                $Mystmt->execute();
                $Mystmt->bind_result($id,$aip,$program,$implementing,$sdate,$cdate,
                $expectedoutput,$funding,$pservice,$mooe,$capital,$totala,$year,$brgycode);
                $Mystmt->fetch();
                $Mystmt->close();
            }
...
<div class="card-header">
    <strong>BDRRMF A. PRE-DISASTER PREPAREDNESS (70%) Information</strong>
</div>
...
<input type="hidden" id="bcode" name="bcode" class="form-control" value="<?php echo $brgycode ?>">
        ...
        <div class="row form-group">
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-8">
                ...
                value="<?php echo $totala ?>" maxlength="12">
            </div>
        </div>
        ...
        <div class="card-footer">
		    <button type="submit" class="btn btn-primary btn-sm" name="updatebdrrmfA">
