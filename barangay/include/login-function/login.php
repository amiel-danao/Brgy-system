...
if(isset($_POST['barangaylogin']))
	{
		...
		$pass = strip_tags(trim($_POST['bpassword']));
...
// 	}
	...
	function checkstatus($usernames)
		{
			global $db;
			   $sqlcheack2 ="SELECT `status` FROM `user_account_tbl` WHERE  `username` = ?";
			   $stmtcheck2 = $db->prepare($sqlcheack2);
		  	   $stmtcheck2->bind_param('s',$usernames);
		  	   $stmtcheck2->execute();
		  	   $stmtcheck2->store_result();
		  	   $stmtcheck2->bind_result($status);
		  	   $stmtcheck2->fetch();
		       $stmtcheck2->close();
