...
if (isset($_POST['updatebrgyofficial']))
   {
    ...
    // else
    //   {
  $mysql = "UPDATE `brgy_official_tbl` SET `first_name`=?, `last_name`=?, `middle_name`=?, `gender`=?, `age`=?, `bod`=?, `birth_of_place`=?, `address`=?, `email`=?, `contact_no`=?, `religion`=?, `civil_status`=?, `highest_education`=?, `course_school`=?, `occupation`=?, `brgy_code`=?, `barangay`=?, `position`=? WHERE `brgy_id`=?";

  $mystmt = $db->prepare($mysql);
  $mystmt->bind_param('ssssisssssssssssssi',$Fname,$Lname,$Mname,$Gender,$age,$dob,$bop,$Address,$email,$contactno,$religion,$status,$education,$school,$Occupation,$brgy_code,$brgy_name,$brgy_official_position,$id);
    // $stmt->execute();
      if($mystmt->execute())
        {
          $mystmt->close();
  ...
  if (!isset($_FILES['new_official_image']['tmp_name']) && !empty($_FILES['new_official_image']['tmp_name'])) 
  ...
  else
    {
      ...
      $brgycode = $_POST['brgycode'];
        if (file_exists("../../../admin/barangayimages/$brgycode/$image"))
          {
            $digit_random_number = mt_rand(100000, 999999);
            $image = $digit_random_number . $image ;  
          }
            move_uploaded_file($tmp_img,"../../../admin/barangayimages/$brgycode/$image");
      
  // if(deleteimage($profileid) == true)
  //     {
        //if
            deleteimage($profileid,$brgycode);
            $mysql = "UPDATE `brgy_official_tbl` SET `picture`=? WHERE `brgy_id`=?";
            $mystmt = $db->prepare($mysql);
          ...
          // $stmt->execute();
            if($mystmt->execute())
            {
              ...
              $mystmt->close();
              $db->close();
              echo "<script>alert('We have Successful Update Profile Image.')</script>";
              echo "<script>window.location.href = '../../barangay_official_list.php'; </script>";
            }
            else
            {
...
function deleteimage($profileid,$brgycode)
    {
      global $db;
          $MYsqls = "SELECT `picture` FROM `brgy_official_tbl` WHERE `brgy_id`=? ";
          ...
          $MYstmts->fetch();
          $MYstmts->close();
          // $Results = $MYstmts->get_result($pictures);
          //  while ($datas = $Results->fetch_assoc())
          // {
          //         $pictures = $datas['picture'];
          // }
          if(file_exists("../../../admin/barangayimages/$brgycode/$pictures"))
          {
           unlink("../../../admin/barangayimages/$brgycode/$pictures");
          }
        //return true;
    }
...
function checkemail($email)
  {
      global $db;
      $sqls = "SELECT `email` FROM `brgy_official_tbl` WHERE `email`=? ";
