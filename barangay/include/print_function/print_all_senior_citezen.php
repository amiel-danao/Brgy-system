...
ob_start();
  ...
  //Long Bond Paper 'mm',array(215.9,279.4)
      ...
      $pdf->SetLineHeight(5);
     if(isset($_GET['printsenior']) && $_GET['printsenior'] != null)
    {
      $code = $_GET['printsenior'];
      // $pdf = new FPDF('P', 'mm', 'A4');
        $pdf->Image('../../logoimg/'. $code.'.png',80,3,40,35);
      $pdf->SetFont('Arial', 'BU', 13);
      
      $pdf->Cell(330 , 6,$_GET['bname'],0,1,'C');
      $pdf->Cell(330 , 6,'Barangay Residents Record Senior Citezen',0,1,'C');
      $pdf->Cell(280 , 15,'',0,1);
      ...
      $pdf->Cell(27 , 10,'college Status',1,1,'C');
        $realage = 0;  
        $senoirage = 60;
        $sql = "SELECT * FROM `residents_tbl` WHERE `brgy_code`=? AND `age` >= {$senoirage} ";
        $stmt = $db->prepare($sql);
